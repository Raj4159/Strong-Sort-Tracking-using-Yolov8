...
csv_filename = 'vehicle_counts.csv'
...
while True:
    ...
    if elapsed_time >= save_interval:
        ...
        with open(csv_filename, mode='a', newline='') as file:
            ...
            writer.writerow({'Time': time.strftime('%Y-%m-%d %H:%M:%S'), 'Direction': 'Up', **dict(zip(required_class_names, copy_up_list))})

            # Write counts for down direction
            writer.writerow({'Time': time.strftime('%Y-%m-%d %H:%M:%S'), 'Direction': 'Down', **dict(zip(required_class_names, copy_down_list))})
